   ...
   /**
	...
	public function ajaxGiveAssignment(Request $request)
	{
      ...
      /*   $class_id = $request->class_id;
		...
		if(!$dateClass_id){
            return json_encode(array('status'=>'error','message'=> Config::get('constants.WebMessageCode.113'))); exit;
        }
		...
		if($assignmentExist){
			return json_encode(array('status'=>'error','message'=> "Assignment already allocated to this class.")); exit;
		}	
		else{
			$s = \DB::table('tbl_classwork_dateclass')->insert(
											['dateclass_id' => $dateClass_id, 'classwork_id' => $classwork_id]
		...
		return json_encode(array('status'=>'success','message'=> 'Assignment Successfully Assign.')); exit;
        ...
        if(!$dateWork_id){
         ...
         $topic_id = $request->topic_id;
			...
			if(count($res)>0)
			{
			   ...
			   return json_encode(array('status'=>'success','topic_link'=>$res_link,'message'=> Config::get('constants.WebMessageCode.131'))); exit;
			}
			else
			{
				 return json_encode(array('status'=>'error','message'=> Config::get('constants.WebMessageCode.113'))); exit;
